...
public class AnalyticsService {
    ...
    public AnalyticsSummaryDto getSummary(User user) {
        ...
        List<Object[]> allUsersRanking = activityLogRepository.getAllUsersFootprintRanking();
        int rank = 1;
        for (int i = 0; i < allUsersRanking.size(); i++) {
            Object[] u = allUsersRanking.get(i);
            if (u[0] != null && u[0].equals(userId)) {
                rank = i + 1;
                break;
            }
        }
        ...
        if (allUsersRanking.size() > 1) {
            // Percentile logic: lower rank means better (lower emissions). 
            // e.g. rank 1 of 10 -> (10-1)/(10-1) * 100 = 100%. Usually it's top (100 - percentile)%
            // Top X% = rank / totalUsers * 100
            percentile = ((double) rank / allUsersRanking.size()) * 100.0;
        } else if (allUsersRanking.size() == 1) {
            percentile = 1.0; // Only user, Top 1%
        }

        List<Object[]> platformAveragesData = activityLogRepository.getPlatformCategoryAverages();
        Map<String, Double> platformAvgMap = new HashMap<>();
        for (Object[] row : platformAveragesData) {
            ActivityCategory cat = (ActivityCategory) row[0];
            Double avg = (Double) row[1];
            if (cat != null && avg != null) {
                platformAvgMap.put(cat.name(), avg);
            }
        }

        List<CategoryAverageDto> categoryComparisons = new ArrayList<>();
        for (Map.Entry<String, Double> entry : categorySums.entrySet()) {
            double userAmt = entry.getValue();
            double platAvg = platformAvgMap.getOrDefault(entry.getKey(), 0.0);
            categoryComparisons.add(CategoryAverageDto.builder()
                    .category(entry.getKey())
                    .userTotal(Math.round(userAmt * 100.0) / 100.0)
                    .platformAverage(Math.round(platAvg * 100.0) / 100.0)
                    .build());
        }

        PeerBenchmarkingDto benchmarking = PeerBenchmarkingDto.builder()
                .userPercentile(Math.round(percentile * 10.0) / 10.0)
                .userRank(rank)
                .totalUsers(allUsersRanking.size())
        ...
        return AnalyticsSummaryDto.builder()
                ...
                .earnedBadges(badges)
                .currentStreak(persistentUser.getCurrentStreak() != null ? persistentUser.getCurrentStreak() : 0)
    ...
    public List<RecommendationDto> getRecommendations(User user) {
        ...
        if (geminiApiKey != null && !geminiApiKey.trim().isEmpty()) {
            try {
                org.springframework.data.domain.Page<ActivityLog> logsPage = activityLogRepository.findByUserIdOrderByLogDateDesc(userId, PageRequest.of(0, 30));
                List<ActivityLog> logs = logsPage.getContent();

                StringBuilder promptBuilder = new StringBuilder();
                promptBuilder.append("You are a carbon footprint and sustainability analyst. ")
                             .append("Based on the user's logged activity history below:\n");

                if (logs.isEmpty()) {
                    promptBuilder.append("- No logged activities yet.\n");
                } else {
                    for (ActivityLog log : logs) {
                        promptBuilder.append(String.format("- Date: %s, Category: %s, Activity Type: %s, Quantity: %f %s, CO2e: %f kg\n",
                            log.getLogDate(), log.getCategory(), log.getActivityType(), log.getQuantity(), log.getUnit(), log.getCalculatedCo2e()));
                    }
                }

                promptBuilder.append("\nGenerate 2-3 personalized, realistic, and highly actionable carbon reduction tips. ")
                             .append("Focus on their high emission categories or suggest general onboarding tips if they have few logs. ")
                             .append("Format the response strictly as a JSON array of objects, containing these fields: ")
                             .append("'category' (MUST be one of: 'TRANSPORT', 'FOOD', 'ELECTRICITY', 'SHOPPING'), ")
                             .append("'tip' (actionable recommendation string), ")
                             .append("'estimatedSavings' (e.g. 'Saves ~X kg CO2e / week'), ")
                             .append("'actionKey' (unique lowercase snake_case code, e.g. 'use_public_transit'). ")
                             .append("Do not wrap the JSON in code blocks (e.g. ```json), return raw JSON text.");
                ...
                String responseJson = restTemplate.postForObject(url, payload, String.class);

                if (responseJson != null) {
                    JsonNode rootNode = objectMapper.readTree(responseJson);
                    String resultJson = rootNode.path("candidates")
                                                .path(0)
                                                .path("content")
                                                .path("parts")
                                                .path(0)
                    ...
                    if (resultJson != null && !resultJson.trim().isEmpty()) {
                        return objectMapper.readValue(resultJson, new TypeReference<List<RecommendationDto>>() {});
                    }
                }
            } catch (Exception e) {
                System.err.println("Gemini recommendation generation failed: " + e.getMessage());
            }
        }
