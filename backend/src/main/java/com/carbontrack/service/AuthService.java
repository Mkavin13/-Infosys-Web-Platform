...
public class AuthService {
    ...
    public AuthResponse googleLogin(com.carbontrack.dto.GoogleLoginRequest request) {
        ...
        // Use request fields if not extracted from JWT
        if (email == null) {
        ...
        // Avoid collision on mock/fallback tokens by making googleSub unique per user email if present
        if (email != null && !email.isBlank() && (googleSub.equals(idToken) || "google_token_123".equals(googleSub))) {
            googleSub = "mock_" + email;
        }
