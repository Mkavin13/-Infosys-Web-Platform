...
public interface ActivityLogRepository extends JpaRepository<ActivityLog, Long> {
    ...
    @Query("SELECT count(a) FROM ActivityLog a WHERE a.user.id = :userId AND a.category = 'TRANSPORT' AND a.activityType IN ('PUBLIC_TRANSIT', 'CAR_ELECTRIC')")
    Long countLowEmissionTransportByUserId(@Param("userId") Long userId);

    @Query("SELECT a.activityType, SUM(a.calculatedCo2e) FROM ActivityLog a " +
           "WHERE a.user.id = :userId AND a.logDate >= :startDate " +
           "GROUP BY a.activityType " +
           "ORDER BY SUM(a.calculatedCo2e) DESC")
    List<Object[]> getActivityTypeEmissionsInPeriod(@Param("userId") Long userId, @Param("startDate") LocalDate startDate);

    @Query("SELECT SUM(a.calculatedCo2e) / COUNT(DISTINCT a.user.id) FROM ActivityLog a")
    Double getPlatformAverageCo2();

    @Query("SELECT a.category, SUM(a.calculatedCo2e) / COUNT(DISTINCT a.user.id) FROM ActivityLog a GROUP BY a.category")
    List<Object[]> getPlatformCategoryAverages();

    @Query("SELECT u.id, COALESCE(SUM(a.calculatedCo2e), 0.0) FROM User u LEFT JOIN ActivityLog a ON u.id = a.user.id GROUP BY u.id ORDER BY COALESCE(SUM(a.calculatedCo2e), 0.0) ASC")
    List<Object[]> getAllUsersFootprintRanking();

    @Query("SELECT COUNT(DISTINCT a.logDate) FROM ActivityLog a WHERE a.user.id = :userId AND a.logDate >= :startDate")
    Long countDistinctLogDatesInPeriod(@Param("userId") Long userId, @Param("startDate") LocalDate startDate);
}
